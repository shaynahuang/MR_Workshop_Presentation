...
## Literature audit, version 2 — 24 September 2026

### Scope and interpretation

This report treats **object manipulation** as acquiring/selecting an object, changing its spatial or intrinsic state, and releasing/confirming it. Spatial transformation includes translation (3 DOF), rotation (3 DOF), and uniform (1 DOF) or independent three-axis scaling (3 DOF): rigid pose has 6 DOF, pose plus uniform scale has 7, and pose plus independent three-axis scale has 9. Intrinsic transformation includes deformation, editing, duplication, grouping, assembly, cutting, and property changes. “Mixed reality” is used broadly enough to include optical/video see-through AR and immersive XR when a technique is foundational to later MR systems.
...
### Search scope and reproducibility limits

This is a preliminary seed set, not a completed systematic review. The source and query lists below describe intended coverage; no reproducible search log, screening flow, or prevalence coding is supplied. Counts: 30 concepts, 23 metaphors, 13 implementation stacks, and 30 core references (including one textbook). The presentation features 22 papers and 16 metaphors.

- Source families: ACM Digital Library, IEEE/ISMAR/VR proceedings, Wiley Computer Graphics Forum, ScienceDirect, publisher/repository copies, and official SDK documentation.
...
| Scaled-world grab | Temporarily scale the world/user relationship so a distant object becomes reachable, then manipulate directly. | Mine, Brooks & Séquin, *Moving Objects in Space: Exploiting Proprioception in Virtual-Environment Interaction*, SIGGRAPH 1997, [DOI](https://doi.org/10.1145/258734.258747) |
...
| Touch + head/device motion | Finger-worn IMU touch input and head motion divide translation DOF in two mappings, Finger Interaction and Head Interaction. | Lee et al., *Virtual Object Manipulation by Combining Touch and Head Interactions…*, 2019, [DOI](https://doi.org/10.3390/app9142933) |
...
| Generic tangible proxy | A reusable sphere/cube/controller is dynamically bound to differently shaped virtual objects. | Englmeier et al., IEEE VR 2020, [DOI](https://doi.org/10.1109/VR46266.2020.00041) |
...
| MR drag-and-drop across real/virtual | Drag objects in a scaled room model to command a Spot robot to pick up and place corresponding physical objects. | *HoloSpot: Intuitive Object Manipulation via Mixed Reality Drag-and-Drop*, 2024, [paper](https://holospot.github.io/static/pdf/holospot_paper.pdf) |
...
| Scaled world / giant user | “The world shrinks, or I become large, so everything is reachable.” | Preserves direct grasp; changes scale/context and may disorient. | Mine, Brooks & Séquin, 1997 |
...
| Touchscreen as window/controller | “I manipulate through a handheld window onto the mixed world.” | Familiar multi-touch and rested input; splits attention and maps 2D to 3D. | Oh et al., 2019; Dual-MR |
...
| Drag-and-drop | “I pick something up here and drop it there,” including across real/virtual representations. | Familiar transfer model; correspondence and target feedback are crucial. | Soler Garcia et al., 2024 |
...
18. Oh et al. (2019). *Virtual Object Manipulation by Combining Touch and Head Interactions for Mobile Augmented Reality*. Applied Sciences. [DOI](https://doi.org/10.3390/app9142933). **MR**
19. Nomura et al. (2020). *Object Manipulation for Perceiving a Sense of Material using User-Perspective Mobile Augmented Reality*. [DOI](https://doi.org/10.3169/mta.8.245). **MR**
20. Englmeier, Doerner, Butz & Höllerer (2020). *A Tangible Spherical Proxy for Object Manipulation in Augmented Reality*. IEEE VR. [DOI](https://doi.org/10.1109/VR46266.2020.00041). **MR**
...
22. Zhou, Williams & Ortega (2022). *Eliciting Multimodal Gesture+Speech Interactions in a Multi-Object Augmented Reality Environment*. [arXiv](https://arxiv.org/abs/2207.12566). **MR**
23. Ishibashi & Kawaguchi. *Object Manipulation Method Using Eye Gaze and Hand-held Controller in AR Space* (2022). VRST. [DOI](https://doi.org/10.1145/3562939.3565659). **MR**
...
26. Hutton Pospick & Suma Rosenberg. *Creating and Manipulating 3D Paths with Mixed Reality Spatial Interfaces* (2023). Frontiers in Virtual Reality. [Article](https://www.frontiersin.org/journals/virtual-reality/articles/10.3389/frvir.2023.1192757/full). **MR**
27. Soler Garcia et al. *HoloSpot: Intuitive Object Manipulation via Mixed Reality Drag-and-Drop* (2024). [Paper](https://holospot.github.io/static/pdf/holospot_paper.pdf). **MR**
28. Liu et al. *Reality Proxy: Fluid Interactions with Real-World Objects in MR via Abstract Representations* (2025). UIST. [DOI](https://doi.org/10.1145/3746059.3747709). **MR**
...
30. Mine, Brooks & Séquin (1997). *Moving Objects in Space: Exploiting Proprioception in Virtual-Environment Interaction*. SIGGRAPH. [DOI](https://doi.org/10.1145/258734.258747). **F**

## 5. Qualitative observations and questions for further review

These are observations about this selected set, not estimates of prevalence across MR research.

1. **Established metaphors recur in selected examples.** Ray selection, remote hand control, and transform handles are reusable ideas; this does not establish that recombination outweighs invention.
2. **Control mapping is a useful design dimension alongside sensing.** Compare near/far, integrated/separated DOF, one/two hands, reference frames, and input-output gain.
3. **Pose control is prominent in this selection.** A systematic search would be needed to assess coverage of deformation, topology, grouping, and semantic editing in head-worn MR.
4. **Remote bare-hand interaction has task-dependent limitations.** Depth control, missing contact forces, tracking noise, fatigue, and coupled motion should be evaluated for each configuration.
5. **Tangibles provide passive haptics and may improve performance in particular tasks.** Benefits depend on the comparator and mapping; prop availability and tracking add deployment costs.
6. **Compare studies with their conditions.** Record distance, target size, DOF, device, constraints, and measures before comparing completion times.
7. **Distinguish representation from actuation.** Reality Proxy supports selection, semantic filtering, and hierarchical navigation through digital stand-ins. HoloBots and HoloSpot use robots to act on the physical environment. Manipulating a digital proxy alone does not move its physical referent.

The presentation's Acquire / Map / Constrain / Confirm structure is an author-defined set of design questions, not a universal sequential model. Constraints may operate continuously or be absent, and confirmation can be implicit.

## 6. Audit flags

- Confirm the bibliographic metadata and final publication venue for items 21 and 29 before formal citation. HoloSpot is cited here as the 2024 preprint, following its [official project citation](https://holospot.github.io/).
...
- [MRTK2 to MRTK3 migration](https://learn.microsoft.com/en-us/windows/mixed-reality/mrtk-unity/mrtk3-overview/architecture/mrtk-v2-to-v3): NearInteractionGrabbable is an MRTK2 component; MRTK3 uses XRI-based interactors/interactables.
- [Reality Proxy, 2025](https://arxiv.org/abs/2507.17248): selection and semantic proxy interaction are distinct from robot actuation.
- [Spherical proxy author thesis](https://www.mmi.ifi.lmu.de/pubdb/publications/pub/englmeier2021diss/englmeier2021diss.pdf): correct DOI is 10.1109/VR46266.2020.00041.
